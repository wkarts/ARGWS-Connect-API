# ARGWS Connect API - AI Agent Guidelines
...
export class ExampleService {
  ...
  public async find(instance: InstanceDto): Promise<ExampleDto | null> {
    try {
      ...
      return result || null; // Return null on not found (Connect pattern)
    } catch (error) {
      this.logger.error('Error finding data:', error);
      return null; // Return null on error (Connect pattern)
    }
  }
}
```
...
```typescript
// CORRECT - ARGWS Connect API pattern (no decorators)
...
export class BadExampleDto {
  @IsString() // ❌ ARGWS Connect API doesn't use decorators
  name: string;
}
```
...
- **Available integrations**: ConnectBot, OpenAI, Dify, Typebot, Chatwoot, Flowise, N8N, ConnectAI
...
- Contact: `security contact: repository security advisories`
