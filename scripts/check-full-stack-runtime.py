...
def main():
    ...
    if not 1 <= args.timeout <= 3600 or (args.port is not None and not 1 <= args.port <= 65535):
    ...
    if args.port is None:
        ...
        ports = [int(port['published']) for name, service in config['services'].items()
                 if name.startswith('api-') for port in service.get('ports', []) if int(port['target']) == 8080]
        if len(ports) != 1:
            raise SystemExit('Porta da API nao identificada no Compose selecionado.')
        args.port = ports[0]
