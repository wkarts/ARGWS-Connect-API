...
const names = ['manifest.json', 'policy.js', 'background.js', 'approve.html', 'approve.js', 'approve.css', 'README.md'];
...
for (const name of names) {
  ...
  local.writeUInt16LE(33,12); local.writeUInt32LE(crc32(body),14); local.writeUInt32LE(body.length,18); local.writeUInt32LE(body.length,22); local.writeUInt16LE(filename.length,26);
  ...
  parts.push(local,filename,body); central.push(index,filename); offset += local.length + filename.length + body.length;
}
