...
Abra `Connect-FindHub-Auth-Assistant-0.1.4-windows-x64.exe`. Clique em **Instalar / atualizar arquivos**; os arquivos embutidos são conferidos e preparados em `%LOCALAPPDATA%\ARGWS\ConnectFindHubAuth\extension`. A atualização mantém a pasta e a identidade da extensão e conserva a versão anterior em `backups`.
