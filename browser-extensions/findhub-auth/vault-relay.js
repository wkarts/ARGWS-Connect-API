// Authenticate this exact tab/frame/document with the worker before forwarding the native callback.
(() => {
  const origin = 'https://accounts.google.com';
  const url = new URL(location.href);
  const kdi = url.searchParams.get('kdi');
  if (window !== window.top || url.origin !== origin || !kdi || url.searchParams.getAll('kdi').length !== 1 ||
      !(url.pathname === '/encryption/unlock/android' ||
        (url.pathname.startsWith('/v3/signin/') && url.searchParams.get('flowName') === 'EncryptionUnlockAndroid'))) return;
  let nonce = null, bound = false, sending = false, sent = false, disposed = false, failed = false;
  const relayError = () => {
    if (!nonce || disposed || sent || failed) return;
    failed = true;
    // Fixed category only. Never expose the callback, PIN, token or arbitrary browser error text.
    void chrome.runtime.sendMessage({ type: 'VAULT_RELAY_ERROR', nonce }).catch(() => {});
  };
  const notify = type => {
    if (!nonce || disposed || failed) return;
    void chrome.runtime.sendMessage({ type, nonce }).then(reply => {
      if (!reply?.ok) relayError();
    }).catch(relayError);
  };
  const closed = () => { notify('VAULT_CLOSED'); };
  const bindPage = () => {
    if (nonce && !disposed) window.postMessage({ source: 'CONNECT_FINDHUB_VAULT_BIND', kdi, nonce }, origin);
  };
  const receive = event => {
    if (!disposed && event.source === window && event.origin === origin &&
        event.data?.source === 'CONNECT_FINDHUB_VAULT_READY' && event.data.kdi === kdi) { bindPage(); return; }
    if (disposed || failed || !nonce || event.source !== window || event.origin !== origin ||
        event.data?.source !== 'CONNECT_FINDHUB_VAULT' || event.data.nonce !== nonce || event.data.kdi !== kdi) return;
    if (event.data.type === 'BOUND') {
      if (!bound) { bound = true; notify('VAULT_READY'); }
      return;
    }
    if (!bound) return;
    if (event.data.type === 'CLOSED') { closed(); return; }
    if (event.data.type !== 'KEYS' || sending || sent || typeof event.data.vaultKeys !== 'string' || event.data.vaultKeys.length > 65536) return;
    sending = true;
    // A rejected promise or reply is not delivery. Never automatically resubmit a key or login artifact.
    void chrome.runtime.sendMessage({ type: 'VAULT_KEYS', nonce, vaultKeys: event.data.vaultKeys }).then(reply => {
      if (disposed) return;
      if (reply?.ok === true) sent = true;
      else relayError();
    }).catch(relayError);
  };
  window.addEventListener('message', receive);
  const hashChanged = () => { if (location.hash === '#close') closed(); };
  window.addEventListener('hashchange', hashChanged);
  window.addEventListener('pagehide', () => {
    disposed = true; nonce = null; window.removeEventListener('message', receive);
    window.removeEventListener('hashchange', hashChanged);
  }, { once: true });
  chrome.runtime.sendMessage({ type: 'VAULT_BIND' }).then(reply => {
    if (disposed || !reply?.ok || reply.kdi !== kdi || typeof reply.nonce !== 'string') return;
    nonce = reply.nonce;
    bindPage();
    hashChanged();
  }).catch(() => { /* Worker deadline or port disconnect terminates a binding that cannot be established. */ });
})();
