...
export class OperationsRouter {
  ...
  constructor() {
    this.router.use((req, res, next) => {
      ...
      if (!expected || !supplied || !timingSafeEqual(hash(expected), hash(supplied))) return res.status(403).json({ error: 'Acesso administrativo necessário.' });
  ...
  private forward(endpoint: string, req: Request, res: Response) {
    ...
    if (!target) return res.status(503).json({ enabled: false, error: 'Monitoramento operacional não habilitado nesta instalação.' });
    if (this.reading >= 4) return res.status(429).json({ error: 'Aguarde a consulta anterior.' });
    const url = new URL(endpoint, target.url);
    const allowed = endpoint === '/history' ? ['from', 'to', 'cursor', 'limit'] : endpoint === '/export' ? ['day', 'format'] : [];
    for (const key of allowed) {
      const value = req.query[key];
      if (value === undefined) continue;
      if (typeof value !== 'string' || value.length > 256) return res.status(400).json({ error: 'Parâmetro inválido.' });
      url.searchParams.set(key, value);
    }
    this.reading++;
    let done = false;
    const release = () => { if (!done) { done = true; this.reading--; } };
    const upstream = http.get(url, { headers: { authorization: `Bearer ${target.token}` }, timeout: 15000 }, (response) => {
      ...
      response.on('error', () => { res.destroy(); release(); });
      response.pipe(res);
    });
