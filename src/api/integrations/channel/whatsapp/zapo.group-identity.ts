...
export class ZapoGroupIdentityCache {
  ...
  constructor(private readonly load: (jid: string) => Promise<GroupIdentity>, private readonly now = Date.now) {}
  invalidate(jid: string): void { this.values.delete(jid); }
  resolve(jid: string): Promise<GroupIdentity | undefined> {
    ...
    const task = this.load(jid).then((value) => {
