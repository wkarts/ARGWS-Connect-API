...
import { ConnectBot, ConnectBotSetting, IntegrationSession } from '@prisma/client';
...
export class ConnectBotService extends BaseChatbotService<ConnectBot, ConnectBotSetting> {
  ...
  constructor(
    ...
    super(waMonitor, prismaRepository, 'ConnectBotService', configService);
  ...
  /**
   * Send a message to the Connect Bot API
   */
  protected async sendMessageToBot(
    instance: any,
    session: IntegrationSession,
    settings: ConnectBotSetting,
    bot: ConnectBot,
    remoteJid: string,
    pushName: string,
    content: string,
    ...
    try {
      ...
      if (this.isAudioMessage(content) && msg) {
        try {
          this.logger.debug(`[ConnectBot] Downloading audio for Whisper transcription`);
          const transcription = await this.openaiService.speechToText(msg, instance);
          if (transcription) {
            payload.query = `[audio] ${transcription}`;
          }
        } catch (err) {
          this.logger.error(`[ConnectBot] Failed to transcribe audio: ${err}`);
        }
      }
      ...
      if (!endpoint) {
        this.logger.error('No Connect Bot endpoint defined');
        return;
      }
      ...
      if (message) {
        ...
        this.logger.warn(`[ConnectBot] No message content received from bot response`);
      }
