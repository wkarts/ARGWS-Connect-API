...
function publishSummary() {
  ...
  const payload = JSON.stringify({
    events: [event, ...(lostBatches ? [{ event: 'telemetry.gap', service: 'api', count: lostBatches }] : [])],
  });
  lostBatches = 0;
  let accepted = false;
  const request = http.request(
    new URL('/events', target.url),
    {
      method: 'POST',
      timeout: 2000,
      headers: {
  ...
  request.once('close', () => {
    publishing = false;
    if (!accepted) lostBatches = Math.min(1000000, lostBatches + 1);
  });
  request.end(payload);
}
...
export function observeOperations(req: Request, res: Response, next: NextFunction) {
  if (!enabled() || req.path === '/health' || req.path === '/metrics' || req.path.startsWith('/operations'))
    return next();
  if (!timer) {
    timer = setInterval(publishSummary, 30000);
    timer.unref();
  }
