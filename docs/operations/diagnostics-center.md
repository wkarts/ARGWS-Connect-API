...
A fila é limitada a 1.000 registros, com gravação assíncrona em lotes, segmentos de até 4 MiB e compactação de segmentos pequenos. O sinalizador Zapo separa dois limites por cliente/minuto: 300 eventos de ciclo de vida e sinalização prioritária, e 100 de `relaylatency`/ACKs rotineiros. Cada grupo registra no máximo um aviso de supressão, identificado por `bucket`. Uma rajada de relay não consome a reserva de oferta, aceite, término, estado e erros. Os IDs de correlação também possuem reservas separadas de até 256 entradas, para que o ruído de relay não retire a correlação de um ACK de aceite.

Erros do frontend têm limite local de 6/minuto e limite adicional no servidor. Saturação, descarte e falha de disco aparecem no painel. `dropped` no manifesto contabiliza perdas no armazenamento central; supressão anterior no observador de sinalização aparece como evento `call.signaling` com `kind=suppressed`. Por isso `dropped=0` isoladamente não garante cobertura integral da sinalização. O sistema prioriza manter chamadas e requisições funcionando mesmo se o diagnóstico falhar.
