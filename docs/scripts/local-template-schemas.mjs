...
export const localTemplateSchemas = {
  ...
  LocalTemplate: {
    type: 'object', description: 'Template persistido da Connect|API. O status é APPROVED e enabled/available controlam separadamente a disponibilidade para envio.',
    properties: { id: { type: 'string' }, ...identity, version, components, enabled: { type: 'boolean' }, available: { type: 'boolean' }, status: { const: 'APPROVED' }, approved: { const: true }, source: { const: 'connectapi_local' }, execution: { const: 'rendered_text' }, category: { const: 'OPENING' }, created_at: { type: 'string', format: 'date-time' }, updated_at: { type: 'string', format: 'date-time' } },
...
export const localTemplateOperations = {
  ...
  'POST /localTemplate/edit/{instanceName}': { summary: 'Editar ou habilitar template', description: 'Nome/idioma imutáveis. version obrigatório evita sobrescrever uma edição concorrente. O status permanece APPROVED; enabled controla disponibilidade.', requestBody: request('LocalTemplateEdit'), responses: { '200': success('LocalTemplate'), ...errors } },
