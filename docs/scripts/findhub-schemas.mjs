...
Object.assign(findHubOperations, {
  ...
  'POST /findhub/auth/browser/exchange/{instanceName}': operation('Validar artefato do login Google', 'Troca uma única vez o artefato temporário autorizado pelo usuário; normaliza o escape URI do cookie sem transformar + em espaço. O campo Email da resposta é opcional: divergência explícita é rejeitada, e uma segunda solicitação confirma o token de serviço ADM para a conta informada antes de prosseguir. Cada solicitação Google tem timeout de 30 segundos; clientes devem permitir pelo menos 65 segundos para esta etapa. Usa HTTPS/1.1 dedicado sem ALPN, com verificação de certificado e hostname habilitada. Falhas retornam mensagem segura com código FH-AUTH-9101 a FH-AUTH-9113, sem resposta bruta, cookies ou tokens. Exige apikey e prova da sessão. Não persiste senha, não aceita callback genérico e não marca a conta como conectada. A resposta fornece somente o endereço de desbloqueio.', ref('FindHubBrowserExchangeResult'), { requestBody: body('FindHubBrowserExchangeRequest') }),
