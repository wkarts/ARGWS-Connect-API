...
const trackingProperties = {
  ...
  reconciliationEnabled: {type:'boolean',default:true,description:'Habilita a camada de reconciliação best effort sem alterar o tracking normal.'},
  reconciliationOnBoot: {type:'boolean',default:true,description:'Procura uma lacuna ao reconectar a conta.'},
  reconciliationPeriodicEnabled: {type:'boolean',default:false,description:'Agenda reconciliações periódicas; permanece opt-in.'},
  reconciliationPeriodSeconds: {type:'integer',minimum:60,maximum:2592000,default:3600},
  reconciliationLookbackHours: {type:'integer',minimum:1,maximum:720,default:48,description:'Janela para detectar também lacunas internas no histórico recente.'},
  reconciliationMinGapSeconds: {type:'integer',minimum:30,maximum:2592000,default:300},
  reconciliationAttempts: {type:'integer',minimum:1,maximum:10,default:3},
};
Object.assign(findHubSchemas, {
...
findHubSchemas.FindHubReconciliationAttempt = {
  type: 'object',
  required: ['attempt', 'durationMs', 'providerReportsDecoded', 'validReports', 'importedReports', 'alreadyStoredReports'],
  properties: {
    attempt: { type: 'integer', minimum: 1, maximum: 10 },
    durationMs: { type: 'integer', minimum: 0 },
    fcmPayloadsReceived: { type: 'integer', minimum: 0 },
    deviceMismatchPayloads: { type: 'integer', minimum: 0 },
    metadataDecodeFailures: { type: 'integer', minimum: 0 },
    providerReportsDecoded: { type: 'integer', minimum: 0 },
    reportsWithEncryptedLocation: { type: 'integer', minimum: 0 },
    reportsWithoutEncryptedLocation: { type: 'integer', minimum: 0 },
    decryptedReports: { type: 'integer', minimum: 0 },
    decryptRejectedReports: { type: 'integer', minimum: 0 },
    decryptErrors: { type: 'integer', minimum: 0 },
    invalidReports: { type: 'integer', minimum: 0 },
    validReports: { type: 'integer', minimum: 0 },
    duplicateValidReports: { type: 'integer', minimum: 0 },
    uniqueValidReports: { type: 'integer', minimum: 0 },
    uniqueReportsReturned: { type: 'integer', minimum: 0 },
    duplicateAcrossAttempts: { type: 'integer', minimum: 0 },
    alreadyStoredReports: { type: 'integer', minimum: 0 },
    importedReports: { type: 'integer', minimum: 0 },
    reportsInTargetRange: { type: 'integer', minimum: 0 },
    reportsOutsideTargetRange: { type: 'integer', minimum: 0 },
    reportsSkippedByRetention: { type: 'integer', minimum: 0 },
    recentReports: { type: 'integer', minimum: 0 },
    networkReports: { type: 'integer', minimum: 0 },
  },
};
findHubSchemas.FindHubReconciliationResult = {
  ...
  required: [
    ...
    'providerReportsDecoded',
    'validProviderReports',
    'alreadyStoredReports',
    'importedReports',
    'reportsInTargetRange',
    'reportsOutsideTargetRange',
  ...
  properties: {
    ...
    trigger: { type: 'string', enum: ['manual', 'boot', 'periodic'] },
    ...
    providerReportsObserved: { type: 'integer', minimum: 0, description:'Compatibilidade: total bruto de relatórios decodificados do provider.' },
    providerReportsDecoded: { type: 'integer', minimum: 0 },
    validProviderReports: { type: 'integer', minimum: 0 },
    uniqueValidProviderReports: { type: 'integer', minimum: 0 },
    alreadyStoredReports: { type: 'integer', minimum: 0 },
    importedReports: { type: 'integer', minimum: 0 },
    reportsInTargetRange: { type: 'integer', minimum: 0 },
    reportsOutsideTargetRange: { type: 'integer', minimum: 0 },
    ...
    sourceCounts: {
      type: 'object',
      properties: { RECENT: { type: 'integer', minimum: 0 }, NETWORK: { type: 'integer', minimum: 0 } },
    },
    decode: {
      type: 'object',
      additionalProperties: false,
    ...
    attempts: { type: 'array', items: ref('FindHubReconciliationAttempt') },
    firstPositionInTargetAt: { type: ['string', 'null'], format: 'date-time' },
    lastPositionInTargetAt: { type: ['string', 'null'], format: 'date-time' },
...
findHubOperations['POST /findhub/positions/reconcile/{deviceId}/{instanceName}'] = operation(
  ...
  'Executa recuperação best effort no Google Find Hub para um dispositivo. Importa e deduplica todos os relatórios válidos RECENT/NETWORK realmente devolvidos pelo provider, inclusive relatórios fora do intervalo usado como métrica. O resultado separa ausência de reports, falha de decode/validação, duplicados, importações fora da faixa e recuperação efetiva. Não interpola posições e não promete reconstrução completa da lacuna.',
  ref('FindHubReconciliationResult'),
  {
    requestBody: {
      required: false,
...
, description: 'IMEI quando o provider Google o entrega; não é inferido.' },
      androidDeviceNumericId: text,
      providerOpaqueId: text,
      providerRegisteredAt: { type: ['string', 'null'], format: 'date-time' },
      providerStatusAt: { type: ['string', 'null'], format: 'date-time' },
