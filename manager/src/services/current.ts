...
async function withInstance<T>(
  ref: string,
  fn: (item: any, name: string, token: string) => Promise<T>,
  refresh = false,
) {
...
const eventConfigKeys = new Set<InstanceConfigKey>([
  'webhook',
  'websocket',
  'rabbitmq',
  'nats',
...
export const current = {
  ...
  async overview(): Promise<Overview> {
    ...
    const connected = items.filter(
      (item) => String(item.connectionStatus || item.status || '').toLowerCase() === 'open',
    ).length;
  ...
  async connectConnection(id: string, data: any = {}) {
    return withInstance(id, async (item, name, token) =>
      api(`/instance/connect/${encodeURIComponent(name)}`, {
        token,
        params:
          data?.pairing && (data?.number || item?.number)
            ? { number: String(data?.number || item.number).replace(/\D/g, '') }
            : undefined,
      }),
    );
  },
  ...
  async restartConnection(id: string) {
    return withInstance(id, async (_item, name, token) =>
      api(`/instance/restart/${encodeURIComponent(name)}`, { method: 'POST', token }),
    );
  },

  async disconnectConnection(id: string) {
    return withInstance(id, async (_item, name, token) =>
      api(`/instance/logout/${encodeURIComponent(name)}`, { method: 'DELETE', token }),
    );
  },

  async removeConnection(id: string) {
    const result = await withInstance(id, async (_item, name, token) =>
      api(`/instance/delete/${encodeURIComponent(name)}`, { method: 'DELETE', token }),
    );
    await rawInstances();
    return result;
  },
  ...
  async migrateProvider(
    ...
    const result = await withInstance(
      id,
      async (_item, name, token) =>
        api<ProviderMigrationResult>(`/instance/migrateProvider/${encodeURIComponent(name)}`, {
          method: 'POST',
          token,
  ...
  async conversations(id: string): Promise<Conversation[]> {
    return withInstance(id, async (_item, name, token) =>
      normalize.conversations(
        await api(`/chat/findChats/${encodeURIComponent(name)}`, {
          method: 'POST',
          token,
          data: { where: {}, sort: 'desc', page: 1, offset: 200 },
        }),
      ),
    );
  },
  ...
  async messages(id: string, ref = ''): Promise<Message[]> {
    return withInstance(id, async (_item, name, token) => {
      ...
      return normalize.messages(
        await api(`/chat/findMessages/${encodeURIComponent(name)}`, {
          method: 'POST',
          token,
          data: { where, sort: 'desc', page: 1, offset: ref ? 250 : 150 },
        }),
      );
    });
  },
  ...
  async sendText(id: string, number: string, text: string) {
    return withInstance(id, async (_item, name, token) =>
      api(`/message/sendText/${encodeURIComponent(name)}`, {
        method: 'POST',
        token,
        data: { number: String(number).replace(/@.+$/, '').replace(/\D/g, ''), text },
      }),
    );
  },
  ...
  async contacts(id: string): Promise<ContactItem[]> {
    return withInstance(id, async (_item, name, token) =>
      normalize.contacts(
        await api(`/chat/findContacts/${encodeURIComponent(name)}`, {
          method: 'POST',
          token,
          data: { where: {}, sort: 'asc', page: 1, offset: 500 },
        }),
      ),
    );
  },
  ...
  async offerCall(id: string, number: string, callDuration?: number) {
    return withInstance(id, async (item, name, token) => {
      const provider = normalize.normalizeProvider(item.integration);
      if (!normalize.providerCapabilities(provider).calls)
        throw new CurrentApiError('Este provider não oferece chamadas nesta versão.', 409);
      const duration = Number(callDuration || 0);
      return api(`/call/offer/${encodeURIComponent(name)}`, {
        method: 'POST',
        token,
        data: { number: String(number).replace(/\D/g, ''), ...(duration > 0 ? { callDuration: duration } : {}) },
      });
    });
  },
  ...
  async voiceMedia(id: string, callId: string, callbacks: VoiceMediaCallbacks = {}) {
    return withInstance(id, async (item, name, token) => {
      const provider = normalize.normalizeProvider(item.integration);
      if (!normalize.providerCapabilities(provider).voice)
        throw new CurrentApiError('Áudio de chamada não está disponível neste provider.', 409);
      ...
      const session = new VoiceMediaSession(
        { apiBaseUrl: runtime.apiBaseUrl, instanceName: name, callId, token: mediaToken },
        callbacks,
      );
      await session.start();
      return session;
        ...
        keys.map(async (key) => {
          try {
            ...
            return {
              ...
              detail:
                rows.length > 0
                  ? `${rows.length} configuração${rows.length === 1 ? '' : 'ões'}`
  ...
  async findIntegrations(id: string, key: IntegrationKey) {
    return withInstance(id, async (_item, name, token) =>
      listFrom(await api(`/${key}/find/${encodeURIComponent(name)}`, { token })),
    );
  },

  async createIntegration(id: string, key: IntegrationKey, data: any) {
    return withInstance(id, async (_item, name, token) =>
      api(`/${key}/create/${encodeURIComponent(name)}`, { method: 'POST', token, data: integrationPayload(key, data) }),
    );
  },

  async updateIntegration(id: string, key: IntegrationKey, integrationRef: string, data: any) {
    return withInstance(id, async (_item, name, token) =>
      api(`/${key}/update/${encodeURIComponent(integrationRef)}/${encodeURIComponent(name)}`, {
        method: 'PUT',
        token,
        data: integrationPayload(key, data),
      }),
    );
  },
  ...
  async deleteIntegration(id: string, key: IntegrationKey, integrationRef: string) {
    return withInstance(id, async (_item, name, token) =>
      api(`/${key}/delete/${encodeURIComponent(integrationRef)}/${encodeURIComponent(name)}`, {
        method: 'DELETE',
        token,
      }),
    );
  ...
  async integrationSettings(id: string, key: IntegrationKey) {
    return withInstance(id, async (_item, name, token) =>
      api(`/${key}/fetchSettings/${encodeURIComponent(name)}`, { token }),
    );
  },

  async saveIntegrationSettings(id: string, key: IntegrationKey, data: any) {
    return withInstance(id, async (_item, name, token) =>
      api(`/${key}/settings/${encodeURIComponent(name)}`, { method: 'POST', token, data }),
    );
  },
  ...
  async integrationSessions(id: string, key: IntegrationKey, integrationRef: string) {
    return withInstance(id, async (_item, name, token) =>
      listFrom(
        await api(`/${key}/fetchSessions/${encodeURIComponent(integrationRef)}/${encodeURIComponent(name)}`, { token }),
      ),
    );
  },
  ...
  async integrationSessionStatus(id: string, key: IntegrationKey, remoteJid: string, status: string) {
    return withInstance(id, async (_item, name, token) =>
      api(`/${key}/changeStatus/${encodeURIComponent(name)}`, { method: 'POST', token, data: { remoteJid, status } }),
    );
  },

  async integrationIgnoreJid(id: string, key: IntegrationKey, remoteJid: string, action: 'add' | 'remove') {
    return withInstance(id, async (_item, name, token) =>
      api(`/${key}/ignoreJid/${encodeURIComponent(name)}`, { method: 'POST', token, data: { remoteJid, action } }),
    );
  },

  async openAiCredentials(id: string) {
    return withInstance(id, async (_item, name, token) =>
      listFrom(await api(`/openai/creds/${encodeURIComponent(name)}`, { token })),
    );
  },

  async createOpenAiCredential(id: string, data: { name: string; apiKey: string }) {
    return withInstance(id, async (_item, name, token) =>
      api(`/openai/creds/${encodeURIComponent(name)}`, { method: 'POST', token, data }),
    );
  },

  async deleteOpenAiCredential(id: string, credentialId: string) {
    return withInstance(id, async (_item, name, token) =>
      api(`/openai/creds/${encodeURIComponent(credentialId)}/${encodeURIComponent(name)}`, { method: 'DELETE', token }),
    );
  },
  ...
  async openAiModels(id: string, credentialId: string) {
    return withInstance(id, async (_item, name, token) =>
      listFrom(
        await api(`/openai/getModels/${encodeURIComponent(name)}`, {
          token,
  ...
  async loadInstanceConfig(id: string, key: InstanceConfigKey) {
    return withInstance(id, async (_item, name, token) =>
      api<any>(`/${key}/find/${encodeURIComponent(name)}`, { token }),
    );
  },
  ...
  async updates() {
    ...
    return {
      ...
      version: info?.version || '',
      available: false,
      message: 'Sua instalação está pronta para receber atualizações quando disponíveis.',
    };
  },

  async users(): Promise<UserItem[]> {
    return [];
  },
  async roles() {
    return [];
  },
  async audit(): Promise<AuditItem[]> {
    return [];
  },
  async security() {
  ...
  async setup() {
    throw new CurrentApiError('Este recurso ainda não está habilitado nesta instalação.', 409);
  },
  async verify() {
    throw new CurrentApiError('Este recurso ainda não está habilitado nesta instalação.', 409);
  },
