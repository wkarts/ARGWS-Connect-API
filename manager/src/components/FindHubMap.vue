...
const tileError = ref(false), retryGeneration = ref(0)
...
const tiles = computed(() => {
  if (!props.position || tileError.value) return []
...
<template>
  <div ref="viewport" class="location-map" tabindex="0" role="region" aria-label="Mapa de localização. Use o scroll do mouse para aproximar ou afastar e as setas para mover o mapa." @pointerdown="start" @pointermove="move" @pointerup="drag=null" @pointercancel="drag=null" @keydown="key">
    ...
    <div v-if="tileError" class="map-tile-error" role="alert"><strong>O provedor não carregou o mapa.</strong><span>A posição continua disponível. Verifique o acesso ao provedor ou configure FINDHUB_MAP_TILE_URL. Nenhuma restrição é contornada automaticamente.</span><button class="btn ghost" @click.stop="retryTiles">Tentar novamente</button></div>
