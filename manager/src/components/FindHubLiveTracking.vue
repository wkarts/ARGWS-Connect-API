...
watch(() => device.value?.lastQuery?.completedAt, () => { if (['new_report','late_report'].includes(device.value?.lastQuery?.status)) { error.value = ''; feedback.value = '' } })
