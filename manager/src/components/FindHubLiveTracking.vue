...
<template><PanelCard title="Localização em tempo real" description="Atualizações por eventos. A posição e o horário exibidos são os efetivamente recebidos, sem simulação de movimento."><div v-if="error" class="alert error" role="alert">{{ error }}</div><div v-else-if="feedback" class="alert" role="status">{{ feedback }}</div><div class="toolbar"><label class="field device-choice"><span>Dispositivo da conta</span><select v-model="selected" :disabled="busy" class="select"><option v-for="d in snapshot.devices" :key="d.id" :value="d.id">{{ d.name }}</option></select></label><span class="muted" role="status">{{ streamState }}</span></div><div v-if="device" class="tracking-tools"><label class="field"><span>Intervalo entre consultas (segundos)</span><input v-model.number="interval" type="number" min="0" step="1" max="86400"/></label><label class="field"><span>Espera manual por nova posição (milissegundos)</span><input v-model.number="timeout" type="number" min="1" max="2147483647" step="1"/></label><div class="toolbar"><button class="btn ghost" :disabled="busy || !snapshot.connected" @click="action('locate')">Localizar agora</button><button class="btn ghost" :disabled="busy || !snapshot.connected" @click="action('start')">{{ device.trackingEnabled ? 'Aplicar intervalo' : 'Iniciar rastreamento' }}</button><button v-if="device.trackingEnabled" class="btn ghost" :disabled="busy" @click="action('stop')">Parar</button></div></div><p class="muted">Recomendado: 60 segundos. 0 consulta novamente ao concluir a anterior; 1, 2 ou mais são aceitos. Falhas e limites do Google podem impor recuo, sem alterar sua configuração.</p><p class="muted">Esta espera afeta somente o retorno do botão “Localizar agora”. O comando é enviado separadamente e uma resposta posterior continua atualizando mapa, latitude e longitude pelo realtime.</p><div class="map-status" role="status"><strong>{{ status }}</strong></div><FindHubPositionDetails :device="device" :stale-after-seconds="snapshot.settings.staleAfterSeconds"/><div v-if="lastQueryMessage" class="alert" role="status">{{ lastQueryMessage }}</div><FindHubMap :key="selected" :position="device?.latestPosition" :trail="trail" :tile-url="snapshot.map?.tileUrl" :avatar-data="avatar" :device-name="device?.name"/><p class="muted top-gap">Uma posição recente não comprova que o aparelho está online. Offline/online só são indicados quando o provedor informa esse estado. O Google pode retornar a última posição conhecida; o intervalo configurado não garante novas coordenadas nesse prazo.</p><p class="muted">Os mapas usam tiles externos OpenStreetMap por padrão; o provedor recebe a área visualizada. A instalação pode configurar um servidor próprio de tiles.</p></PanelCard></template>
