...
const lastQueryMessage = computed(() => queryMessage(device.value?.lastQuery, device.value?.lastReceivedAt))
watch(() => device.value?.latestPosition?.timestamp, (next, previous) => {
 if (previous && next && Date.parse(next) > Date.parse(previous)) { error.value = ''; feedback.value = '' }
})
