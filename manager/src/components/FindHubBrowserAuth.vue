  ...
  try {
    ...
    port.onMessage.addListener((message: any) => {
      ...
      void (async () => {
        ...
        if (message.type === 'WAITING_WEB_SESSION') stage.value = 'Conclua o login web e a confirmação em duas etapas na página Google. Depois será aberto o desbloqueio dos dados do Find Hub.'
        if (message.type === 'WAITING_VAULT_KEY') stage.value = 'Conclua as etapas exibidas pelo Google. Ele pode pedir confirmação em duas etapas antes do PIN de bloqueio do aparelho.'
