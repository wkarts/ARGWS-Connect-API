(() => {
  ...
  function policy() {
    ...
    if (!value.interactionsV2 || Number(value.interactionsV2.version) !== 2) value.interactionsV2 = { version: 2, items: [] };
  ...
  function actions() { return parse($('actionsJsonInput')?.value, { bindings: [] }); }
  function instanceName() { return String($('instanceSelect')?.value || '').trim(); }
  function apiKey() { return String($('apiKeyInput')?.value || '').trim(); }
  function templateName() { return String($('nameInput')?.value || '').trim(); }
  function language() { return String($('languageInput')?.value || 'pt_BR').trim() || 'pt_BR'; }
  function sampleVariables() { return parse($('variablesInput')?.value, {}); }
  ...
  async function api(path, options = {}) {
    ...
    if (!response.ok) throw new Error(data?.response?.message || data?.message || data?.error?.message || text || `HTTP ${response.status}`);
    return data;
  }
  ...
  function injectUI() {
    ...
    root.querySelectorAll('[data-p6-view]').forEach((button) => button.addEventListener('click', () => {
      root.querySelectorAll('[data-p6-view]').forEach((item) => item.classList.toggle('active', item === button));
      root.querySelectorAll('[data-p6-panel]').forEach((item) => item.classList.toggle('active', item.dataset.p6Panel === button.dataset.p6View));
    }));
  ...
  function interactionItems(p = policy()) { return p.interactionsV2.items; }

  function addInteraction(type) {
    const p = policy();
    const number = p.interactionsV2.items.length + 1;
    const item = type === 'LIST'
      ? { type: 'LIST', id: `list_${number}`, title: 'Escolha uma opção', body: 'Selecione um item', buttonText: 'Ver opções', sections: [{ title: 'Opções', rows: [{ id: 'option_1', title: 'Opção 1', capture: { path: 'selection' } }] }] }
      : { type: 'CHOICE', id: `choice_${number}`, title: 'Confirma?', body: 'Selecione uma opção', mode: 'SINGLE', options: [{ id: 'yes', title: 'Sim', capture: { path: 'decision', value: true } }, { id: 'no', title: 'Não', capture: { path: 'decision', value: false } }] };
  ...
  function setInteractionField(field, value) {
    ...
    if (!item) return;
    if (value === '' && ['footer'].includes(field)) delete item[field]; else item[field] = value;
    writePolicy(p);
  }
  ...
  function renderInteractionList() {
    ...
    if (!root) return;
    if (!list.length) { root.innerHTML = '<div class="p6-empty">Nenhuma interação v2.</div>'; return; }
    if (state.interactionIndex >= list.length) state.interactionIndex = list.length - 1;
    root.innerHTML = list.map((item, index) => `<button class="p6-item ${index === state.interactionIndex ? 'active' : ''}" data-p6-interaction="${index}" type="button"><strong>${esc(item.id || `interação ${index + 1}`)}</strong><span>${esc(item.type)} · ${item.source?.path ? `dinâmica: ${esc(item.source.path)}` : 'estática'}</span></button>`).join('');
    root.querySelectorAll('[data-p6-interaction]').forEach((node) => node.addEventListener('click', () => { state.interactionIndex = Number(node.dataset.p6Interaction); renderInteractions(); }));
  }
  ...
  function renderInteractionEditor() {
    ...
    if (!root) return;
    if (!item) { root.innerHTML = '<div class="p6-empty">Adicione uma LIST ou CHOICE.</div>'; return; }
    ...
    if (item.type === 'LIST') {
      ...
      collection = sections.map((section, sectionIndex) => `<div class="p6-subcard"><div class="p6-subhead"><strong>Seção ${sectionIndex + 1}</strong><input data-p6-section-title="${sectionIndex}" value="${esc(section.title || '')}" placeholder="Título da seção" style="max-width:260px" /></div>${(section.rows || []).map((row, rowIndex) => rowEditor(row, rowIndex, 'LIST', sectionIndex)).join('') || '<div class="p6-empty">Sem linhas.</div>'}<button class="p6-button" data-p6-add-row="${sectionIndex}" type="button">+ Linha</button></div>`).join('');
  ...
  function bindInteractionEditor() {
    ...
    document.querySelectorAll('#p6InteractionEditor [data-p6-field]').forEach((input) => input.addEventListener('input', () => setInteractionField(input.dataset.p6Field, input.value)));
    document.querySelectorAll('#p6InteractionEditor [data-p6-source]').forEach((input) => input.addEventListener('input', () => {
      const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; if (!item) return;
      item.source ||= {}; const value = input.value.trim(); if (value) item.source[input.dataset.p6Source] = value; else delete item.source[input.dataset.p6Source];
      if (!item.source.path && !item.source.id && !item.source.title) delete item.source;
      writePolicy(p); renderMapperPreview();
    }));
    $('p6InteractionEditor')?.querySelector('[data-p6-source-capture]')?.addEventListener('input', (event) => {
      const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; if (!item) return; item.source ||= {};
      const value = event.target.value.trim(); if (value) item.source.capture = { ...(item.source.capture || {}), path: value }; else delete item.source.capture; writePolicy(p);
    });
    document.querySelectorAll('#p6InteractionEditor [data-p6-source-binding]').forEach((input) => input.addEventListener('input', () => {
      const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; if (!item) return; item.source ||= {}; item.source.binding ||= {};
      const field = input.dataset.p6SourceBinding;
      if (field === 'keepSessionOpen') item.source.binding[field] = input.value === 'true'; else if (input.value) item.source.binding[field] = input.value; else delete item.source.binding[field];
      if ((!item.source.binding.type || item.source.binding.type === 'NONE') && !item.source.binding.key) delete item.source.binding;
      writePolicy(p);
    }));
    document.querySelectorAll('#p6InteractionEditor [data-p6-section-title]').forEach((input) => input.addEventListener('input', () => {
      const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; const section = item?.sections?.[Number(input.dataset.p6SectionTitle)]; if (!section) return; section.title = input.value; writePolicy(p);
    }));
    $('p6AddSection')?.addEventListener('click', () => { const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; item.sections ||= []; item.sections.push({ title: `Seção ${item.sections.length + 1}`, rows: [] }); writePolicy(p, true); });
    document.querySelectorAll('#p6InteractionEditor [data-p6-add-row]').forEach((button) => button.addEventListener('click', () => { const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; const section = item.sections[Number(button.dataset.p6AddRow)]; section.rows ||= []; const n = section.rows.length + 1; section.rows.push({ id: `option_${n}`, title: `Opção ${n}` }); writePolicy(p, true); }));
    $('p6AddOption')?.addEventListener('click', () => { const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; item.options ||= []; const n = item.options.length + 1; item.options.push({ id: `option_${n}`, title: `Opção ${n}` }); writePolicy(p, true); });
    document.querySelectorAll('#p6InteractionEditor [data-p6-row]').forEach((card) => {
      const kind = card.dataset.p6Kind; const rowIndex = Number(card.dataset.p6Row); const sectionIndex = Number(card.dataset.p6Section || 0);
      const rowFor = (p) => kind === 'LIST' ? p.interactionsV2.items[state.interactionIndex]?.sections?.[sectionIndex]?.rows?.[rowIndex] : p.interactionsV2.items[state.interactionIndex]?.options?.[rowIndex];
      card.querySelector('[data-p6-remove-row]')?.addEventListener('click', () => { const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; if (kind === 'LIST') item.sections[sectionIndex].rows.splice(rowIndex, 1); else item.options.splice(rowIndex, 1); writePolicy(p, true); });
      card.querySelectorAll('[data-p6-row-field]').forEach((input) => input.addEventListener('input', () => {
        const p = policy(); const row = rowFor(p); if (!row) return; const field = input.dataset.p6RowField; const value = input.value;
        if (field === 'capturePath') { if (value.trim()) row.capture = { ...(row.capture || {}), path: value.trim() }; else delete row.capture; }
        else if (field === 'captureValue') { if (!value.trim()) { if (row.capture) delete row.capture.value; } else { row.capture ||= { path: 'selection' }; try { row.capture.value = JSON.parse(value); } catch { row.capture.value = value; } } }
        else if (field === 'bindingType') { row.binding ||= {}; row.binding.type = value; if (value === 'NONE' && !row.binding.key) delete row.binding; }
        else if (field === 'bindingKey') { row.binding ||= { type: 'ACTION' }; row.binding.key = value.trim(); }
        else if (field === 'confirm') { row.binding ||= { type: 'NONE' }; row.binding.confirmOnInteraction = value === 'true'; }
        else { if (value) row[field] = value; else delete row[field]; }
        writePolicy(p);
      }));
    });
  }
  ...
  function renderMapperPreview() {
    const node = $('p6MapperPreview'); if (!node) return;
    const p = policy(); const item = p.interactionsV2.items[state.interactionIndex]; const source = item?.source;
    if (!source?.path) { node.textContent = 'Defina um Array path para visualizar dados de amostra.'; return; }
    const value = readPath(sampleVariables(), source.path);
    node.textContent = Array.isArray(value) ? `${value.length} item(ns) encontrados em variables.${source.path}. O backend resolve {{item.*}} no envio.` : `O path ${source.path} ainda não aponta para um array nas variáveis de amostra.`;
  }

  function readPath(source, path) {
    const normalized = String(path || '').replace(/^\$\.?/, '').replace(/^result\.?/, '').replace(/\[(\d+)\]/g, '.$1');
    return normalized.split('.').filter(Boolean).reduce((value, key) => value == null ? undefined : value[key], source);
  }

  function renderTransportPreview() {
    const node = $('p6InteractionTransport'); if (!node) return;
    const item = interactionItems()[state.interactionIndex]; const plan = state.lastPreview?.transport?.interactions || [];
    const current = plan.find((entry) => entry.id === item?.id);
    node.innerHTML = current ? `<div><strong>Provider:</strong> ${esc(state.lastPreview?.provider || '')}</div><div><strong>Transporte:</strong> ${esc(current.mode)} · ${esc(current.compatibilityTransport || 'nativo')}</div>${(current.warnings || []).map((warning) => `<div>${esc(warning)}</div>`).join('')}` : '<div>O preview do provider será exibido aqui após conectar a instância.</div>';
  }

  function renderInteractions() { renderInteractionList(); renderInteractionEditor(); }

  function addApp() {
    const p = policy(); const n = p.microApps.apps.length + 1;
    p.microApps.apps.push({ key: `app_${n}`, title: `Micro App ${n}`, description: 'Fluxo conversacional multipágina', startPage: 'start', ttlSeconds: 900, accessMode: 'CONVERSATION_SESSION', pages: [{ key: 'start', title: 'Início', description: 'Preencha os dados', components: [{ type: 'INPUT', id: 'name', label: 'Nome' }] }] });
    state.appIndex = p.microApps.apps.length - 1; state.pageIndex = 0; writePolicy(p, true);
  }

  function addCheckinPreset() {
    const p = policy();
    p.microApps.apps.push({ key: 'checkin', title: 'Check-in com localização', description: 'Captura dados e valida presença em geofence.', startPage: 'identify', ttlSeconds: 900, accessMode: 'CONVERSATION_SESSION', pages: [{ key: 'identify', title: 'Identificação', description: 'Informe os dados para continuar.', captureRoot: 'form', components: [{ type: 'INPUT', id: 'name', label: 'Nome' }, { type: 'INPUT', id: 'document', label: 'Documento' }], next: 'location' }, { key: 'location', title: 'Confirmar localização', description: 'Autorize o GPS para validar sua presença.', location: { mode: 'REQUIRED_AUTO', capturePath: 'visit.location', policy: { enabled: true, allowedSources: ['MICRO_APP_GPS'], maxAccuracyMeters: 100, geofences: [], outsideGeofence: 'BLOCK' } }, components: [{ type: 'LOCATION', id: 'location', label: 'Localização atual' }] }] });
    state.appIndex = p.microApps.apps.length - 1; state.pageIndex = 0; writePolicy(p, true);
  }

  function renderAppList() {
    const p = policy(); const apps = p.microApps.apps; const root = $('p6AppList'); if (!root) return;
    if (!apps.length) { root.innerHTML = '<div class="p6-empty">Nenhum Micro App.<br><button id="p6PresetCheckin" class="p6-button" type="button" style="margin-top:8px">Criar preset de check-in</button></div>'; $('p6PresetCheckin')?.addEventListener('click', addCheckinPreset); return; }
    if (state.appIndex >= apps.length) state.appIndex = apps.length - 1;
    root.innerHTML = apps.map((app, index) => `<button class="p6-item ${index === state.appIndex ? 'active' : ''}" data-p6-app="${index}" type="button"><strong>${esc(app.title || app.key)}</strong><span>${esc(app.key)} · ${(app.pages || []).length} página(s)</span></button>`).join('');
    root.querySelectorAll('[data-p6-app]').forEach((node) => node.addEventListener('click', () => { state.appIndex = Number(node.dataset.p6App); state.pageIndex = 0; renderApps(); }));
  }

  function componentSummary(components) { return (components || []).map((component) => component.type || 'INPUT').join(', ') || 'sem componentes'; }

  function renderAppEditor() {
    const p = policy(); const app = p.microApps.apps[state.appIndex]; const root = $('p6AppEditor'); if (!root) return;
    if (!app) { root.innerHTML = '<div class="p6-empty">Adicione um Micro App ou use o preset de check-in.</div>'; return; }
    app.pages ||= []; if (state.pageIndex >= app.pages.length) state.pageIndex = Math.max(0, app.pages.length - 1); const page = app.pages[state.pageIndex];
  ...
  function pageEditor(page, app) {
    const location = page.location || {}; const geofence = location.policy?.geofences?.[0] || {};
    return `<div class="p6-subcard"><div class="p6-subhead"><strong>Página · ${esc(page.key)}</strong><button id="p6RemovePage" class="p6-button danger" type="button">Remover</button></div><div class="p6-row three"><label><span>Page key</span><input data-p6-page-field="key" value="${esc(page.key || '')}" /></label><label><span>Título</span><input data-p6-page-field="title" value="${esc(page.title || '')}" /></label><label><span>Próxima página</span><select data-p6-page-field="next"><option value="">Concluir</option>${app.pages.filter((candidate) => candidate !== page).map((candidate) => `<option ${candidate.key === page.next ? 'selected' : ''}>${esc(candidate.key)}</option>`).join('')}</select></label></div><div class="p6-row"><label><span>Descrição</span><input data-p6-page-field="description" value="${esc(page.description || '')}" /></label><label><span>Capture root</span><input data-p6-page-field="captureRoot" value="${esc(page.captureRoot || '')}" placeholder="form" /></label></div><div class="p6-row three"><label><span>Localização</span><select data-p6-location="mode"><option ${!location.mode || location.mode === 'DISABLED' ? 'selected' : ''}>DISABLED</option><option ${location.mode === 'OPTIONAL' ? 'selected' : ''}>OPTIONAL</option><option ${location.mode === 'REQUIRED' ? 'selected' : ''}>REQUIRED</option><option ${location.mode === 'REQUIRED_AUTO' ? 'selected' : ''}>REQUIRED_AUTO</option></select></label><label><span>Capture path GPS</span><input data-p6-location="capturePath" value="${esc(location.capturePath || 'location')}" /></label><label><span>Precisão máx. m</span><input data-p6-location-policy="maxAccuracyMeters" type="number" value="${esc(location.policy?.maxAccuracyMeters || 100)}" /></label></div><div class="p6-row three"><label><span>Geofence lat</span><input data-p6-geofence="latitude" type="number" step="any" value="${esc(geofence.latitude ?? '')}" /></label><label><span>Geofence lon</span><input data-p6-geofence="longitude" type="number" step="any" value="${esc(geofence.longitude ?? '')}" /></label><label><span>Raio m</span><input data-p6-geofence="radiusMeters" type="number" value="${esc(geofence.radiusMeters || 500)}" /></label></div><div class="p6-row"><label><span>Operação LOAD</span><input data-p6-operation="load" value="${esc(page.load ? `${page.load.type}:${page.load.key}` : '')}" placeholder="ACTION:catalog.list" /></label><label><span>Operação SUBMIT</span><input data-p6-operation="submit" value="${esc(page.submit ? `${page.submit.type}:${page.submit.key}` : '')}" placeholder="RECIPE:checkin.complete" /></label></div><div class="p6-help">Componentes: ${esc(componentSummary(page.components))}. Para INPUT/SELECT/RADIO/TABLE e transições condicionais use o JSON técnico enquanto o designer visual avançado permanece compatível com o mesmo contrato.</div></div>`;
  }

  function bindAppEditor() {
    const pNow = policy(); const appNow = pNow.microApps.apps[state.appIndex]; if (!appNow) return;
    $('p6RemoveApp')?.addEventListener('click', () => { const p = policy(); p.microApps.apps.splice(state.appIndex, 1); state.appIndex = Math.max(0, state.appIndex - 1); state.pageIndex = 0; writePolicy(p, true); });
    $('p6AddPage')?.addEventListener('click', () => { const p = policy(); const app = p.microApps.apps[state.appIndex]; const n = app.pages.length + 1; app.pages.push({ key: `page_${n}`, title: `Página ${n}`, components: [] }); state.pageIndex = app.pages.length - 1; writePolicy(p, true); });
    document.querySelectorAll('#p6AppEditor [data-p6-page]').forEach((node) => node.addEventListener('click', () => { state.pageIndex = Number(node.dataset.p6Page); renderApps(); }));
    document.querySelectorAll('#p6AppEditor [data-p6-app-field]').forEach((input) => input.addEventListener('input', () => { const p = policy(); const app = p.microApps.apps[state.appIndex]; const field = input.dataset.p6AppField; app[field] = field === 'ttlSeconds' ? Number(input.value || 900) : input.value; writePolicy(p); renderAppList(); }));
    const pageFor = (p) => p.microApps.apps[state.appIndex]?.pages?.[state.pageIndex];
    document.querySelectorAll('#p6AppEditor [data-p6-page-field]').forEach((input) => input.addEventListener('input', () => { const p = policy(); const page = pageFor(p); if (!page) return; const oldKey = page.key; if (input.value) page[input.dataset.p6PageField] = input.value; else delete page[input.dataset.p6PageField]; const app = p.microApps.apps[state.appIndex]; if (input.dataset.p6PageField === 'key' && app.startPage === oldKey) app.startPage = input.value; writePolicy(p); }));
    $('p6RemovePage')?.addEventListener('click', () => { const p = policy(); const app = p.microApps.apps[state.appIndex]; const removed = app.pages.splice(state.pageIndex, 1)[0]; if (removed?.key === app.startPage) app.startPage = app.pages[0]?.key || ''; state.pageIndex = Math.max(0, state.pageIndex - 1); writePolicy(p, true); });
    document.querySelectorAll('#p6AppEditor [data-p6-location]').forEach((input) => input.addEventListener('input', () => { const p = policy(); const page = pageFor(p); page.location ||= {}; const field = input.dataset.p6Location; if (field === 'mode' && input.value === 'DISABLED') { page.location.mode = 'DISABLED'; } else page.location[field] = input.value; writePolicy(p); }));
    document.querySelectorAll('#p6AppEditor [data-p6-location-policy]').forEach((input) => input.addEventListener('input', () => { const p = policy(); const page = pageFor(p); page.location ||= {}; page.location.policy ||= { enabled: true, allowedSources: ['MICRO_APP_GPS'], geofences: [], outsideGeofence: 'BLOCK' }; page.location.policy[input.dataset.p6LocationPolicy] = Number(input.value || 0); writePolicy(p); }));
    document.querySelectorAll('#p6AppEditor [data-p6-geofence]').forEach((input) => input.addEventListener('input', () => { const p = policy(); const page = pageFor(p); page.location ||= {}; page.location.policy ||= { enabled: true, allowedSources: ['MICRO_APP_GPS'], outsideGeofence: 'BLOCK' }; page.location.policy.geofences ||= []; page.location.policy.geofences[0] ||= { id: 'default', name: 'Área permitida', latitude: 0, longitude: 0, radiusMeters: 500 }; page.location.policy.geofences[0][input.dataset.p6Geofence] = Number(input.value || 0); writePolicy(p); }));
    document.querySelectorAll('#p6AppEditor [data-p6-operation]').forEach((input) => input.addEventListener('change', () => { const p = policy(); const page = pageFor(p); const field = input.dataset.p6Operation; const [type, ...rest] = input.value.split(':'); const key = rest.join(':').trim(); if (!key || !['ACTION','RECIPE'].includes(type.toUpperCase())) delete page[field]; else page[field] = { type: type.toUpperCase(), key }; writePolicy(p); }));
    $('p6ApplyAppJson')?.addEventListener('click', () => { try { const next = JSON.parse($('p6AppJson').value); const p = policy(); p.microApps.apps[state.appIndex] = next; state.pageIndex = 0; writePolicy(p, true); toast('JSON do Micro App aplicado.'); } catch (error) { toast(`JSON inválido: ${error.message}`, true); } });
  ...
  async function createTestSession() {
    try {
      ...
      const p = policy(); const app = p.microApps.apps[state.appIndex]; if (!app) throw new Error('Selecione um Micro App.');
      const number = String($('testNumberInput')?.value || '').trim(); if (!number) throw new Error('Informe o número na aba Teste.');
      const result = await api(`/micro-app/session/${encodeURIComponent(instanceName())}`, { method: 'POST', body: { templateName: templateName(), language: language(), appKey: app.key, number, variables: sampleVariables(), ttlSeconds: Number(app.ttlSeconds || 900) } });
      $('p6AppResult').textContent = JSON.stringify(result, null, 2);
      if (result?.url) window.open(result.url, '_blank', 'noopener,noreferrer');
      toast('Sessão Micro App criada.');
    } catch (error) { $('p6AppResult').textContent = `ERRO\n${error.message}`; toast(error.message, true); }
  }

  function renderApps() { renderAppList(); renderAppEditor(); }

  function captureGps() {
    const out = $('p6LocationResult'); if (!out) return;
    if (!navigator.geolocation) { out.textContent = 'Geolocalização não suportada neste navegador.'; return; }
    out.textContent = 'Obtendo localização...';
    navigator.geolocation.getCurrentPosition((position) => { out.textContent = JSON.stringify({ source: 'MICRO_APP_GPS', latitude: position.coords.latitude, longitude: position.coords.longitude, accuracy: position.coords.accuracy, capturedAt: new Date(position.timestamp).toISOString() }, null, 2); }, (error) => { out.textContent = `ERRO\n${error.message}`; }, { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 });
  }

  function renderAll() { renderInteractions(); renderApps(); }

  function reloadFromEditor() {
    if (state.writing) return;
    state.interactionIndex = 0; state.appIndex = 0; state.pageIndex = 0; renderAll();
  }

  injectStyle();
  injectUI();
  renderAll();
  window.addEventListener('argws:template-preview', (event) => { state.lastPreview = event.detail; renderTransportPreview(); });
