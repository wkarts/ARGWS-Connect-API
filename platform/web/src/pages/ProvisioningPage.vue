...
<template>
  ...
  <section>
    ...
    <div class="table-wrap"><table class="table"><thead><tr><th>Cliente</th><th>Operação</th><th>Etapa</th><th>Progresso</th><th>Estado</th><th>Início</th><th></th></tr></thead><tbody>
      <tr v-for="item in jobs" :key="item.id" :class="targetJobId===item.id?'bg-teal-50/70':''"><td><p class="font-semibold">{{tenantName(item.tenant_id)}}</p><p class="text-[11px] text-slate-400">{{item.tenant_id.slice(0,8)}}…</p></td><td>{{operationLabel(item.operation)}}</td><td>{{stepLabel(item.current_step)}}</td><td><div class="w-36"><div class="h-1.5 rounded-full bg-slate-100"><div class="h-1.5 rounded-full bg-teal-600" :style="{width:`${Math.min(100,item.progress||0)}%`}"/></div><p class="mt-1 text-[11px] text-slate-400">{{item.progress||0}}% · {{item.attempts}} tentativa(s)</p></div></td><td><StatusBadge :status="item.status"/></td><td>{{new Date(item.started_at||item.created_at).toLocaleString('pt-BR')}}</td><td><button class="btn-secondary !px-3 !py-2" @click="openJob(item)">Eventos</button></td></tr>
