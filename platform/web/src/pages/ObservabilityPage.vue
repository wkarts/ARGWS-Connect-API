...
const sourceDefinitions:SourceDefinition[]=[
  // Mantemos os aliases Connect|API antigos e reconhecemos também o contrato
  // de nomes atual <resource>-argws-connect-<deployment> sem renomear stacks.
  {key:'api',label:'Connect API / Platform API',category:'Aplicação',icon:Server,matches:['connect-api','api-argws-connect-','platform-api-argws-connect-'],structured:['connect-api','platform-api','api']},
  {key:'workers',label:'Workers Connect|API',category:'Processamento',icon:Workflow,matches:['connect-worker-','platform-worker-argws-connect-'],structured:['worker','notifications','events','webhooks','automations','connect']},
  {key:'beat',label:'Agendador de tarefas',category:'Processamento',icon:Timer,matches:['connect-beat','platform-scheduler-argws-connect-'],structured:['scheduler','beat']},
  {key:'gateway',label:'Gateway / Interface Platform',category:'Web',icon:Network,matches:['connect-gateway','connect-web','platform-gateway-argws-connect-','platform-web-argws-connect-'],structured:['frontend','control-web','tenant-web','gateway']},
  {key:'postgres',label:'PostgreSQL',category:'Dados',icon:Database,matches:['connect-postgres','postgres-argws-connect-','platform-postgres-argws-connect-'],structured:['database','postgres']},
  {key:'redis',label:'Redis',category:'Dados',icon:Database,matches:['connect-redis','redis-argws-connect-'],structured:['redis']},
  {key:'rabbitmq',label:'RabbitMQ',category:'Mensageria',icon:MessageSquare,matches:['connect-rabbitmq','rabbitmq-argws-connect-'],structured:['rabbitmq','queue']},
  {key:'storage',label:'MinIO / S3',category:'Armazenamento',icon:HardDrive,matches:['connect-minio','minio-argws-connect-'],structured:['storage','s3','minio']},
  {key:'monitoring',label:'Logs e monitoramento',category:'Infraestrutura',icon:Activity,matches:['connect-prometheus','connect-grafana','connect-log-agent','connect-docker-proxy','platform-prometheus-argws-connect-','platform-grafana-argws-connect-','platform-log-agent-argws-connect-','platform-docker-proxy-argws-connect-'],structured:['observability','system','prometheus','grafana']},
  {key:'edge',label:'DNS / ACME / CloudPanel',category:'Infraestrutura',icon:ShieldCheck,matches:['connect-acme','connect-cloudpanel-agent','platform-acme-argws-connect-','platform-cloudpanel-agent-argws-connect-'],structured:['domain','dns','ssl','acme','cloudflare','cloudpanel']},
]
...
function serviceLabel(value?:string|null):string{
  ...
  if(key.startsWith('platform-api-argws-connect-'))return 'Platform API'
  if(key.startsWith('api-argws-connect-'))return 'Connect API'
  if(key.startsWith('platform-worker-backups-argws-connect-'))return 'Worker de backup'
  if(key.startsWith('platform-worker-argws-connect-'))return 'Worker Platform'
  if(key.startsWith('platform-scheduler-argws-connect-'))return 'Agendador'
  if(key.startsWith('platform-web-argws-connect-'))return 'Interface Platform'
  if(key.startsWith('platform-gateway-argws-connect-'))return 'Gateway Platform'
  if(key.startsWith('platform-postgres-argws-connect-'))return 'PostgreSQL Platform'
  if(key.startsWith('postgres-argws-connect-'))return 'PostgreSQL Engine'
  if(key.startsWith('redis-argws-connect-'))return 'Redis'
  if(key.startsWith('rabbitmq-argws-connect-'))return 'RabbitMQ'
  if(key.startsWith('minio-argws-connect-'))return 'MinIO / S3'
  if(key.startsWith('platform-log-agent-argws-connect-'))return 'Agente de logs'
  if(key.startsWith('platform-docker-proxy-argws-connect-'))return 'Proxy Docker somente leitura'
  if(key.startsWith('platform-prometheus-argws-connect-'))return 'Prometheus'
  if(key.startsWith('platform-grafana-argws-connect-'))return 'Grafana'
  if(key.startsWith('platform-acme-argws-connect-'))return 'ACME DNS-01'
  if(key.startsWith('platform-cloudpanel-agent-argws-connect-'))return 'CloudPanel Agent'
  return raw||'Serviço'
}
...
async function loadRuntime(){
  ...
  try{
    ...
    if(!selectedContainer.value&&services.value.length)selectedContainer.value=services.value.find(item=>String(item.service||'').startsWith('platform-api-argws-connect-'))?.id||services.value.find(item=>item.service==='connect-api')?.id||services.value[0].id
...
<template>
  ...
  <section v-if="summary" class="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
    ...
    <div class="card"><div class="flex items-start justify-between"><div><p class="text-xs font-semibold uppercase text-slate-400">Stack</p><p class="mt-2 text-2xl font-bold">{{summary.containers.running}} / {{summary.containers.total}}</p><p class="mt-1 text-xs text-slate-400">serviços em execução</p></div><Server :size="22" class="text-blue-600"/></div></div>
  ...
  <template v-else-if="tab==='console'">
    ...
    <template v-else>
      ...
      <div class="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]"><aside class="space-y-2"><button v-for="item in services" :key="item.id" class="w-full rounded-xl border bg-white p-3 text-left transition" :class="selectedContainer===item.id?'border-blue-400 ring-1 ring-blue-300':'border-slate-200 hover:border-slate-300'" @click="selectedContainer=item.id;loadContainerLogs()"><div class="flex items-center justify-between gap-2"><span class="truncate text-sm font-semibold">{{serviceLabel(item.service||item.name)}}</span><span class="h-2.5 w-2.5 shrink-0 rounded-full" :class="stateStatus(item)==='ACTIVE'?'bg-emerald-500':stateStatus(item)==='ERROR'?'bg-rose-500':'bg-amber-500'"/></div><p class="mt-1 truncate text-xs text-slate-400">{{statusLabel(item.health||item.state)}} · {{item.restart_count}} reinício(s)</p></button></aside><section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft"><div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 p-4"><div><p class="text-xs font-bold uppercase tracking-wide text-blue-700">Console somente leitura</p><h2 class="mt-1 font-bold">{{selectedService?serviceLabel(selectedService.service||selectedService.name):'Selecione um serviço'}}</h2><p v-if="selectedService" class="mt-1 text-xs text-slate-400">{{statusLabel(selectedService.health||selectedService.state)}} · {{selectedService.image}}</p></div><StatusBadge v-if="selectedService" :status="stateStatus(selectedService)"/></div><pre class="scroll-clean max-h-[68vh] min-h-[520px] overflow-auto whitespace-pre-wrap break-all bg-slate-950 p-4 font-mono text-[11px] leading-5 text-slate-200">{{containerLines.length?containerLines.join('\n'):'Nenhuma linha retornada para esta consulta.'}}</pre></section></div>
    </template>
  </template>
