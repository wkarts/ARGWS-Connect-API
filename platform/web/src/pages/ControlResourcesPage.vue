...
<template>
  <div class="space-y-5">
          ...
          <table class="w-full text-left text-xs">
            ...
            <tbody>
              <tr v-for="bucket in buckets" :key="bucket.name" class="border-b border-slate-100 last:border-0">
                <td class="p-2"><button class="font-semibold text-teal-700 hover:underline" @click="loadBucketObjects(bucket.name)">{{ bucket.name }}</button></td>
    ...
    <section v-else class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      <RouterLink v-for="item in catalog" :key="item.code" :to="item.path" class="card group flex items-center gap-3 p-4 transition hover:-translate-y-0.5 hover:shadow-md"><div class="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-700 group-hover:bg-teal-50 group-hover:text-teal-700"><ServerCog :size="19"/></div><div class="min-w-0 flex-1"><p class="font-semibold text-slate-900">{{ item.name }}</p><p class="text-xs text-slate-500">{{ item.category }} · {{ item.management }}</p></div></RouterLink>
    </section>
  </div>
</template>
