...
<template>
  ...
  <section class="card mb-6 border-teal-100">
    <div class="flex flex-wrap items-start justify-between gap-4"><div><div class="flex items-center gap-2"><component :is="landingForm.enabled?Eye:EyeOff" :size="20" class="text-teal-700"/><h2 class="text-lg font-bold">Landing page pública</h2></div><p class="mt-1 max-w-3xl text-sm text-slate-500">Controle o que pode ser mostrado publicamente. URLs administrativas, infraestrutura, providers, repositórios e detalhes técnicos não são publicados pela landing.</p></div><StatusBadge :status="landingForm.enabled?'ACTIVE':'DISABLED'"/></div>
  ...
  <section class="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
    <div class="space-y-5">
      <div v-for="category in categories" :key="category" class="card !p-0 overflow-hidden">
        <div class="flex items-center gap-3 border-b border-slate-200 px-5 py-4"><Settings2 :size="20" class="text-teal-700" /><h2 class="font-semibold">{{ categoryOptions.find(option=>option.value===category)?.label||category }}</h2></div>
